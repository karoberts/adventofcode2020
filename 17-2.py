...
with open("17.txt") as f:
    ...
    for line in (l.strip() for l in f):
        x = 0
        for c in line:
            grid[(x,y,z,w)] = True if c == '#' else False
            x += 1
        y += 1

neighbors = set(itertools.product(range(-1,2), repeat=4))
...
def count_neighbors(g, p):
    return sum( 1 for n in neighbors if g[ (n[0] + p[0], n[1] + p[1], n[2] + p[2], n[3] + p[3]) ] )
...
for round in range(0, 6):
    new_grid = grid.copy()
    for w in range(max_w[0], max_w[1] + 1):
        for z in range(max_z[0], max_z[1] + 1):
            for y in range(max_y[0], max_y[1] + 1):
                for x in range(max_x[0], max_x[1] + 1):
                    p = (x,y,z,w)
